...
%
function [weights,delta]=backprop_faulty(data,W,labels,lr)
    ...
    % output
    delta{2}=errors{2} .* logistic_der(O{2});
    ...
    for j=1:size(W{1},1)
        ...
        del(j)=err(j)*logistic_der(O{1}(j));
    end
    ...
    for i=1:2
		...
		for n=1:size(O{i},2)
			...
			for j=1:length(inputs)
				W{i}(n,j) = W{i}(n,j)+ lr * delta{i}(j)*inputs(j);
            end
            % this is the update rule for the BIAS (in the LAST element
            % of our matrix)
            W{i}(n,end) = W{i}(n,end)+ lr * delta{i}(j);
        end
    end
